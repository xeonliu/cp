...
依赖信息在 [`pyproject.toml`](file:///Users/liu/CP/pyproject.toml) 中已经列出，建议使用你熟悉的工具（如 `uv`、`pip` 或 `poetry`）安装。
...
3. **调整导入设置**
   - 在右侧“导入设置”中：
     - 选择“按日期组织文件”与否；
     - 设置“日期格式”（如 `YYYY-MM-DD`、`YYYY/MM/DD` 等）；
     - 选择时间源：
       - `文件修改时间`（默认）
