...
import psutil
import rawpy
from PyQt6.QtCore import QDir, QObject, QPoint, QRect, QSize, Qt, QThread, QTimer, pyqtSignal
from PyQt6.QtGui import QColor, QFileSystemModel, QIcon, QImage, QPixmap
...
/* 按钮 */
QPushButton { background-color: #333; border: 1px solid #555; border-radius: 3px;
              padding: 5px 15px; color: #ccc; }
...
from cp.worker import ImportWorker, ScannerWorker, ThumbnailWorker
from cp.view.widgets import AutoHeightListWidget, DateSection


# --- 主窗口 ---
class CardCopierWindow(QMainWindow):
    ...
    )  # 文件列表, 目标路径, 模式, 是否按日期, 日期格式, 时间源, 自定义模板

    def __init__(self):
        super().__init__()
        self.setWindowTitle("CP - CardCopier")
    ...
    def create_top_bar(self):
        ...
        for i, text in enumerate(modes):
            ...
            btn.setStyleSheet("""
                QPushButton { border: none; font-weight: bold; color: #999; font-size: 14px;
    ...
    def create_left_panel(self):
        ...
        dev_title.setStyleSheet(
            "font-weight: bold; background-color: #333; color: #eee; padding-left: 5px;"
        )
        ...
        self.fs_model.setFilter(
            QDir.Filter.NoDotAndDotDot | QDir.Filter.AllDirs | QDir.Filter.Drives
        )
    ...
    def create_right_panel(self):
        ...
        self.date_format_combo.addItems(
            ["YYYY-MM-DD", "YYYY/MM/DD", "YYYY-MM", "YYYY/MM", "YYYYMMDD", "YYYY"]
        )
        ...
        self.template_input.setPlaceholderText(
            "例: {year}/{month:02d}/{day:02d} 或 {year}-{month}-{day}"
        )
        self.template_input.setStyleSheet(
            "background-color: #222; border: 1px solid #444; padding: 4px;"
        )
    ...
    def refresh_devices(self):
        ...
        for p in partitions:
            ...
            if is_removable:
                ...
                item.setIcon(
                    self.style().standardIcon(getattr(self.style().StandardPixmap, icon_name))
                )
        ...
        for section in self.date_sections.values():
            ...
            for i in range(list_widget.count()):
                ...
                if self.organize_checkbox.isChecked():
                    # 计算目标路径
                    path_str = self.get_path_from_file(
                        file_path, use_template=bool(self.custom_template)
                    )
    ...
    def on_browse_target(self):
        ...
        dir_path = QFileDialog.getExistingDirectory(
            self, "选择导入目标文件夹", self.target_directory
        )
    ...
    def on_import_clicked(self):
        ...
        if organize_by_date:
            ...
            time_label = "拍摄时间" if time_source == "exif" else "文件修改时间"
            msg = (
                f"即将导入 {len(files_to_import)} 个文件\n"
                f"{preview_fmt}\n"
                f"时间源: {time_label}\n"
                f"目标: {self.target_directory}"
            )
        else:
        ...
        reply = QMessageBox.question(
            self, "确认导入", msg, QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )
    ...
    def on_import_finished(self, success):
        ...
        if success:
            ...
            QMessageBox.information(
                self, "成功", f"已导入 {self.last_import_count} 个文件到\n{self.target_directory}"
            )
    ...
    def on_tree_item_changed(self, item, column):
        ...
        if item.font(0).italic() and item.data(0, Qt.ItemDataRole.UserRole):
            ...
            for file_path, grid_item in self.path_to_item.items():
                file_dir = self.get_path_from_file(
                    file_path, use_template=bool(self.custom_template)
                )
    ...
    def populate_tree(
        self, parent_item, directory, max_depth=2, current_depth=0, existing_only=False
    ):
        """递归填充树项目，显示现有的目录和文件"""
        ...
        try:
            ...
            for entry in entries:
                ...
                if os.path.isdir(full_path) and not entry.startswith("."):
                    ...
                    # 递归添加子文件夹
                    self.populate_tree(
                        item, full_path, max_depth, current_depth + 1, existing_only=existing_only
                    )
